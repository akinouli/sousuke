...
function addScheduleIcons(dayElement, date, data) {
  ...
  if (startDate && date.getTime() === startDate.getTime()) {
    iconArea.appendChild(
      createScheduleIcon("icon/cal1_start.png", "作業開始日"),
    );
  }
  ...
  if (completionDate && date.getTime() === completionDate.getTime()) {
    iconArea.appendChild(
      createScheduleIcon("icon/cal2_completed.png", "作品完成日"),
    );
  }
  ...
  if (deadline && date.getTime() === deadline.getTime()) {
    iconArea.appendChild(
      createScheduleIcon("icon/cal3_deadline.png", "締切日"),
    );
  }
...
function renderHorizontalScheduleCalendar() {
  ...
  weeks.forEach((week) => {
    const includesCurrentMonth = week.some(
    ...
    if (includesCurrentMonth) {
      appendScheduleWeek(
        ...
        week,
        scheduleCalendarData,
        isFirstDisplayedWeek ? week[0] : null,
      );
